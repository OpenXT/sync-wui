...
return declare("citrix.common._BoundContainerMixin", null, {
    ...
    bind: function(source, rootNode, includeTemplates) {
        this._binding = true;
    ...
    unbind: function(rootNode, includeTemplates) {
        var ignoreAttr = (includeTemplates) ? "" : "templateType";
        var attribute = "name";
        var obj = {};
        dojo.forEach(this.getDescendants(rootNode, ignoreAttr), function(widget) {
            if (widget.disabled || !widget[attribute] || widget["bindIgnore"]) {
                return;
            }
            var name = widget[attribute];
            // Single value widget (checkbox, radio, or plain <input> type widget)
            var value = widget.get('value');
            if(widget.declaredClass == "citrix.common.EditableWidget" && widget.editing) {
                value = widget.get("editingValue");
            }
